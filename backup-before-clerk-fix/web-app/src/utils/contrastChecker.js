...
export function generateContrastRecommendations(ratio, isLargeText) {
  ...
  if (ratio < targetAA) {
    recommendations.push(
      `Contrast ratio ${ratio.toFixed(2)} fails WCAG AA standard (${targetAA} required)`,
    );
    recommendations.push('Consider darkening text or lightening background');
  } else if (ratio < targetAAA) {
    recommendations.push(
      `Meets WCAG AA but fails AAA standard (${targetAAA} required for enhanced accessibility)`,
    );
  } else {
...
export function auditPageContrast() {
  const textElements = document.querySelectorAll(
    'p, h1, h2, h3, h4, h5, h6, span, a, button, label, input, textarea, select, li',
  );
  const results = [];
  const summary = {
    total: 0,
  ...
  textElements.forEach(element => {
    if (element.offsetParent !== null && element.textContent.trim()) {
      // Only visible elements with text
...
export class ContrastMonitor {
  ...
  checkNewElements() {
    const elements = document.querySelectorAll(
      '[data-contrast-check="false"], :not([data-contrast-check])',
    );
