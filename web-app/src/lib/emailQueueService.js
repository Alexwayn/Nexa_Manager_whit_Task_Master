...
class EmailQueueService {
   ...
   */
  startQueue(intervalMs = 60000) { // Check every minute by default
  ...
  async processQueue() {
    ...
    try {
      ...
      const dueCampaigns = scheduledCampaigns.filter(campaign => 
  ...
  async getQueueMetrics() {
    try {
      ...
      const overdueCampaigns = scheduledCampaigns.filter(campaign => 
        new Date(campaign.scheduled_at) < now
      );
      
      const upcomingCampaigns = scheduledCampaigns.filter(campaign => 
        new Date(campaign.scheduled_at) >= now
      );
      ...
      return {
        ...
        data: {
          ...
          stats: this.processingStats,
          nextCampaign: upcomingCampaigns.length > 0 ? 
