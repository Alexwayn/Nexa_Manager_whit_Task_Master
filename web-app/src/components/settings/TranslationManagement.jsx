...
const TranslationManagement = () => {
  ...
  const namespaces = [
    'common', 'calendar', 'navigation', 'forms', 'buttons', 'receipts',
    'inventory', 'documentation', 'dashboard', 'invoices', 'clients',
    'quotes', 'reports', 'settings', 'analytics', 'transactions',
    'documents', 'email', 'scan', 'voice'
  ];
    ...
    try {
      ...
      for (const namespace of namespaces) {
        ...
        stats[namespace] = {
          ...
          coverage: baseKeys.length > 0 ? ((targetKeys.length / baseKeys.length) * 100).toFixed(1) : 100
        };
      }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (importData.translations) {
          ...
          i18n.addResourceBundle(selectedLanguage, selectedNamespace, importData.translations, true, true);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {t('settings:translationManagement.description')}
          </p>
        </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <span>
                  {t('settings:translationManagement.translations')} 
                ...
                <Card key={namespace}>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={stats.coverage >= 95 ? "default" : stats.coverage >= 80 ? "secondary" : "destructive"}>
                        {stats.coverage}%
                      </Badge>
                    </div>
                  </CardHeader>
            ...
            <CardContent className="space-y-4">
              ...
              {auditReport && (
                <div className="space-y-4">
                  ...
                  {/* Issues by Language */}
                  {auditReport.languages && Object.entries(auditReport.languages).map(([lang, langReport]) => (
                    <Card key={lang}>
                      <CardHeader>
                        <CardTitle className="flex items-center gap-2">
                          <Globe className="h-5 w-5" />
                          {lang.toUpperCase()}
                          <Badge variant={langReport.issues?.length > 0 ? "destructive" : "default"}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <Select value={selectedNamespace} onValueChange={setSelectedNamespace}>
                  <SelectTrigger className="w-full sm:w-auto">
                    <SelectValue placeholder={t('settings:translationManagement.selectNamespace')} />
                  </SelectTrigger>
                  <SelectContent>
                    {namespaces.map(ns => (
                      <SelectItem key={ns} value={ns}>
                        ...
                        <div className="space-y-2">
                          {qualityReport.warnings.map((warning, index) => (
                            <Alert key={index}>
                              <AlertTriangle className="h-4 w-4" />
                              <AlertDescription>
                                <span className="font-mono text-sm">{warning.key}</span>: {warning.message}
                              </AlertDescription>
                            </Alert>
                          ))}
                        </div>
