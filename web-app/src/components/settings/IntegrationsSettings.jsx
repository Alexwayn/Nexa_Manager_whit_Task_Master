...
export default function IntegrationsSettings({ showNotification }) {
  ...
  const saveIntegrationConfig = async () => {
    try {
      ...
      await integrationsService.updateIntegrationConfig(
        configModal.service,
        configModal.data
      );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {t('integrations.apiKeys.description', 'Generate and manage API keys for accessing your data programmatically.')}
          </p>
        </div>
        <button
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                      <span className="text-sm text-gray-600 font-mono">
                        {showApiKey[apiKey.id] ? apiKey.key || apiKey.key_prefix : apiKey.key_prefix}
                      </span>
                      <button
                    ...
                    <div className="flex flex-wrap gap-4 text-xs text-gray-500">
                      ...
                      {apiKey.expires_at && (
                        <span className="text-amber-600">Expires: {formatDate(apiKey.expires_at)}</span>
                      )}
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewApiKeyData(prev => ({ ...prev, rateLimitRequests: parseInt(e.target.value) }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    min="1"
                    max="10000"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewApiKeyData(prev => ({ ...prev, expiresAt: e.target.value || null }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
              </div>
  ...
  const renderIntegrations = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          {t('integrations.thirdParty.description', 'Connect with external services to enhance your workflow.')}
        </p>
      </div>
              ...
              <div key={service} className="border border-gray-100 rounded-lg p-4 hover:border-gray-200 transition-colors">
                ...
                <div className="space-y-2 mb-4">
                  <p className="text-xs text-gray-500">
                    Status: <span className={config.connected ? 'text-green-600 font-medium' : 'text-gray-500'}>
                      {config.status}
                    </span>
                  </p>
  ...
  const renderActivityTab = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Monitor API usage and integration sync history.
        </p>
      </div>
                ...
                {activityHistory.map((activity) => (
                  <tr key={activity.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        activity.activity_status === 'success' ? 'bg-green-100 text-green-800' :
                        activity.activity_status === 'error' ? 'bg-red-100 text-red-800' :
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {t('integrations.description', 'Manage API access and connect with third-party services.')}
        </p>
      </div>
