...
const AccessibilitySettings = () => {
  ...
  const runAccessibilityTest = async () => {
    ...
    try {
      ...
      announceToScreenReader(`Accessibility test completed. Found ${newReport.summary.violationCount} violations`, 'polite');
    } catch (error) {
  ...
  const runQuickAudit = () => {
    ...
    announceToScreenReader(`Quick audit completed. Overall score: ${quickReport.overallScore}%`, 'polite');
  };
    ...
    <div ref={contentRef} className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600">
          {t('accessibility.description', 'Monitor and improve the accessibility of your application with automated testing and real-time monitoring.')}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <span className="text-sm font-medium text-blue-900">
            {isTestRunning ? t('accessibility.testing', 'Testing...') : t('accessibility.runTest', 'Run Full Test')}
          </span>
        </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className={`p-4 rounded-lg ${getScoreBgColor(currentReport.overallScore || 0)}`}>
              <div className="text-center">
                <div className={`text-2xl font-bold ${getScoreColor(currentReport.overallScore || 0)}`}>
              ...
              <div className="space-y-2">
                {currentReport.results.slice(0, 3).map((result, index) => (
                  <div key={index} className="p-3 bg-blue-50 rounded-md border border-blue-200">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-blue-900">{result.suggestions?.[0] || 'Improvement suggestion available'}</span>
            ...
            {testHistory.slice(0, 5).map((historyReport, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                <div className="flex items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center ${getScoreBgColor(historyReport.overallScore || 0)}`}>
                    <span className={`text-xs font-medium ${getScoreColor(historyReport.overallScore || 0)}`}>
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-xs text-gray-500">
                      {historyReport.summary?.violationCount || historyReport.totalIssues || 0} violations found
                    </p>
                  </div>
                </div>
                <button
