...
const ProfileSection = ({ setNotification }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      setNotification({ show: true, message: t('profile.alerts.updateSuccess'), type: 'success' });
    }
  };
  ...
  const handleAvatarUpload = async (file) => {
    ...
    if (result) {
      // The avatar URL is automatically updated in the profile
      setNotification({
        show: true,
        message: t('profile.alerts.avatarUpdateSuccess'),
        type: 'success',
      });
      // Force a page reload to see the updated avatar
      setTimeout(() => {
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-gray-500">{t('profile.email.help')}</p>
        </div>
        <div>
