...
export default function EmailSettings({ showNotification }) {
  ...
  const handleTestEmailConfiguration = async () => {
    try {
      setIsSaving(true);
      const result = await EmailSettingsService.testEmailConfiguration(smtpTestEmail, emailSettings);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-6">
          {t('email.providers.description', 'Configure your email service provider for sending emails.')}
        </p>
      </div>
  ...
  const renderNotificationSettings = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mb-6">
          {t('email.notifications.description', 'Choose which email notifications you want to receive.')}
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {[
            { key: 'invoice_sent', label: 'Invoice Sent', desc: 'When an invoice is sent to a client' },
            { key: 'invoice_viewed', label: 'Invoice Viewed', desc: 'When a client views an invoice' },
            { key: 'invoice_paid', label: 'Invoice Paid', desc: 'When an invoice is marked as paid' },
            { key: 'invoice_overdue', label: 'Invoice Overdue', desc: 'When an invoice becomes overdue' },
            { key: 'invoice_cancelled', label: 'Invoice Cancelled', desc: 'When an invoice is cancelled' }
          ].map(({ key, label, desc }) => (
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {[
            { key: 'payment_received', label: 'Payment Received', desc: 'When a payment is received' },
            { key: 'payment_failed', label: 'Payment Failed', desc: 'When a payment attempt fails' },
            { key: 'payment_refunded', label: 'Payment Refunded', desc: 'When a payment is refunded' },
            { key: 'payment_reminder_sent', label: 'Reminder Sent', desc: 'When payment reminders are sent' }
          ].map(({ key, label, desc }) => (
            <div key={key} className="flex items-center justify-between">
              <div>
                <label className="text-sm font-medium text-gray-700">{label}</label>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {[
            ...
            { key: 'quote_accepted', label: 'Quote Accepted', desc: 'When a client accepts a quote' },
            { key: 'quote_declined', label: 'Quote Declined', desc: 'When a client declines a quote' },
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {[
            { key: 'system_backup', label: 'System Backup', desc: 'Backup completion notifications' },
            { key: 'system_maintenance', label: 'Maintenance', desc: 'System maintenance updates' },
            { key: 'system_security', label: 'Security Alerts', desc: 'Security-related notifications' },
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-600">
              {t('email.templates.description', 'Customize your email templates for different types of communications.')}
            </p>
          </div>
          <button
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs ${
                      EmailSettingsService.templateCategories[template.category]?.color === 'blue' ? 'bg-blue-100 text-blue-800' :
                      EmailSettingsService.templateCategories[template.category]?.color === 'green' ? 'bg-green-100 text-green-800' :
                      EmailSettingsService.templateCategories[template.category]?.color === 'purple' ? 'bg-purple-100 text-purple-800' :
