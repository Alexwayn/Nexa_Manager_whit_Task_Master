  ...
  return (
    <div className="fixed top-4 right-4 bg-white border border-gray-300 rounded-lg shadow-lg p-4 z-50 max-w-md">
      ...
      <div className="space-y-2 text-sm">
        <div><strong>Lingua attuale:</strong> {debugInfo.currentLanguage}</div>
        <div><strong>i18n Ready:</strong> {debugInfo.isReady ? '✅' : '❌'}</div>
        <div><strong>localStorage:</strong> {debugInfo.localStorage || 'non impostato'}</div>
        <div><strong>Lingue disponibili:</strong> {debugInfo.availableLanguages?.join(', ')}</div>
      </div>
