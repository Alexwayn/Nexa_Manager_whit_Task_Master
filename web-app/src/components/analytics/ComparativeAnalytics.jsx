...
const ComparativeAnalytics = ({ 
  ...
  useEffect(() => {
    ...
    const loadComparativeData = async () => {
      ...
      try {
        const [currentFinancial, comparisonFinancial, currentClients, comparisonClients] = await Promise.all([
          ...
          clientService.getClientMetrics ? 
            clientService.getClientMetrics(
              new Date(comparisonDateRange.start), 
              new Date(comparisonDateRange.end)
            ) : Promise.resolve({ success: true, data: { total: 35, active: 32, newThisMonth: 2 } })
        ]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {t('analytics:charts.comparisonDesc')}
          </p>
        </div>
      ...
      <div className="flex items-center justify-center space-x-8 py-4 bg-gray-50 rounded-lg">
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">
            {new Date(currentDateRange.start).toLocaleDateString()} - {new Date(currentDateRange.end).toLocaleDateString()}
          </div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-gray-500">
            {new Date(comparisonDateRange.start).toLocaleDateString()} - {new Date(comparisonDateRange.end).toLocaleDateString()}
          </div>
        </div>
      </div>
    </div>
