          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-semibold text-gray-900">
                {quote ? `${t('quotes.quote', 'Quote')} #${quote.quote_number}` : t('quotes.loading', 'Loading Quote...')}
              </h2>
            ...
            {quote && !isLoading && !error && (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <dl className="space-y-2">
                            <div>
                              <dt className="text-sm font-medium text-gray-500">{t('quotes.client', 'Client')}</dt>
                              <dd className="text-sm text-gray-900">{quote.client_name}</dd>
                            </div>
                            <div>
                              <dt className="text-sm font-medium text-gray-500">{t('quotes.created_date', 'Created')}</dt>
                              <dd className="text-sm text-gray-900">{formatDate(quote.created_at)}</dd>
                            </div>
                            <div>
                              <dt className="text-sm font-medium text-gray-500">{t('quotes.valid_until', 'Valid Until')}</dt>
                              <dd className="text-sm text-gray-900">{formatDate(quote.valid_until)}</dd>
                            </div>
                            {quote.acceptance_deadline && (
                              <div>
                                <dt className="text-sm font-medium text-gray-500">{t('quotes.acceptance_deadline', 'Acceptance Deadline')}</dt>
                                <dd className="text-sm text-gray-900">{formatDate(quote.acceptance_deadline)}</dd>
                              </div>
                            )}
                          </dl>
                        </div>
                        ...
                        <div>
                          ...
                          <dl className="space-y-2">
                            <div>
                              <dt className="text-sm font-medium text-gray-500">{t('quotes.subtotal', 'Subtotal')}</dt>
                              <dd className="text-sm text-gray-900">{formatCurrency(quote.subtotal)}</dd>
                            </div>
                            <div>
                              <dt className="text-sm font-medium text-gray-500">{t('quotes.tax', 'Tax')}</dt>
                              <dd className="text-sm text-gray-900">{formatCurrency(quote.tax_amount)}</dd>
                            </div>
                            <div className="border-t pt-2">
                              <dt className="text-sm font-medium text-gray-900">{t('quotes.total', 'Total')}</dt>
                  ...
                  {/* History Tab */}
                  {activeTab === 'history' && (
                    <QuoteStatusHistory quoteId={quote.id} />
                  )}
                        ...
                        <div>
                          ...
                          <div className="bg-green-50 border border-green-200 rounded-md p-4">
                            ...
                            <div className="text-xs text-green-700">
                              {t('quotes.signature.signed_on', 'Signed on')}: {formatDate(quote.signature_date)}
                            </div>
                          </div>
                        </div>
                      ) : (
