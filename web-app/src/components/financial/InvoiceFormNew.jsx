...
const InvoiceForm = ({ 
  ...
  const validateForm = () => {
    ...
    formData.items.forEach((item, index) => {
      if (!item.description) errors[`item_${index}_description`] = t('form.itemDescriptionRequired');
      if (!item.quantity || item.quantity <= 0) errors[`item_${index}_quantity`] = t('form.validQuantityRequired');
      if (!item.unit_price || item.unit_price <= 0) errors[`item_${index}_unit_price`] = t('form.validUnitPriceRequired');
    });
  ...
  const handleSave = async (asDraft = false) => {
    try {
      ...
      let result;
      if (isEditMode && invoice) {
        result = await InvoiceService.updateInvoice(invoice.id, dataToSave, dataToSave.items, dataToSave.user_id);
      } else {
        result = await InvoiceService.createInvoice(dataToSave, dataToSave.items, dataToSave.user_id);
      }
    ...
    <div className={`bg-white rounded-lg ${className}`}>
      ...
      <div className="p-6 space-y-6">
              ...
              <div key={item.id} className="grid grid-cols-12 gap-4 items-end p-4 bg-gray-50 rounded-lg">
                ...
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    step="0.01"
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
                      validationErrors[`item_${index}_unit_price`] ? 'border-red-500' : 'border-gray-300'
                    }`}
                  />
                </div>
        ...
        <div className="flex justify-end space-x-4 pt-6 border-t">
          ...
          >
            {isSaving ? t('form.saving') : (isEditMode ? t('form.updateInvoice') : t('form.createInvoiceButton'))}
          </button>
        </div>
      </div>
    </div>
