...
export const createMockAuthClient = (initialUser = null, initialSession = null) => {
  ...
  const mockAuth = {
    // User management
    getUser: jest.fn(() => 
      Promise.resolve(createSupabaseResponse({ user: currentUser }))
    ),
    
    getSession: jest.fn(() => 
      Promise.resolve(createSupabaseResponse({ session: currentSession }))
    ),
    ...
    signInWithPassword: jest.fn(({ email, password }) => {
      if (email === 'test@example.com' && password === 'password') {
        ...
        return Promise.resolve(createSupabaseResponse({ user: currentUser, session: currentSession }));
      }
      return Promise.resolve(createSupabaseResponse(null, createSupabaseError('Invalid credentials')));
    }),
    ...
    resetPasswordForEmail: jest.fn((email) => 
      Promise.resolve(createSupabaseResponse(null))
    ),
    ...
    updateUser: jest.fn((attributes) => {
      ...
      return Promise.resolve(createSupabaseResponse(null, createSupabaseError('Not authenticated')));
    }),
  ...
  const mockStorage = {
    from: jest.fn((bucket) => ({
      ...
      upload: jest.fn((path, file, options = {}) => {
        if (file.size > 50 * 1024 * 1024) { // 50MB limit
          return Promise.resolve(createSupabaseResponse(
            null, 
            createSupabaseError('File too large')
          ));
        }
        return Promise.resolve(createSupabaseResponse({
          path: `${bucket}/${path}`,
      ...
      download: jest.fn((path) => 
        Promise.resolve(createSupabaseResponse(
...
export const createMockSupabaseClient = (options = {}) => {
  ...
  const mockAuth = createMockAuthClient(
    authOptions.initialUser,
    authOptions.initialSession
  );
  ...
  const createQueryBuilder = (tableName) => {
    ...
    const queryBuilder = {
      ...
      single: jest.fn(() => {
        return Promise.resolve(createSupabaseResponse(
          databaseOptions.mockData?.[tableName]?.[0] || null
        ));
      }),
