...
const getDisplayName = (client) => {
  return client.company_name || client.name || client.first_name + ' ' + client.last_name || 'Unknown Client';
};
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-200">
        <div className="flex items-center space-x-2">
          ...
          <button className="p-2 text-gray-400 hover:text-purple-500 rounded-md hover:bg-gray-100" title={t('actions.createInvoice')} onClick={() => onCreateInvoice(client)}>
...
function Clients() {
  ...
  const filteredAndSortedClients = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'last_contact':
          return new Date(b.last_contact || b.created_at || 0) - new Date(a.last_contact || a.created_at || 0);
        case 'revenue':
  ...
  const handleExportList = useCallback(async () => {
    try {
      ...
      notify.success(t('actions.exportData') + ' - ' + t('pagination.showing', 'Exported {{count}} clients', { count: clientsToExport.length }));
    } catch (error) {
  ...
  const handleGenerateReport = useCallback(async () => {
    try {
      ...
      const reportData = {
        ...
        averageRevenue: clients.length > 0 ? clients.reduce((sum, client) => sum + (client.revenue || 0), 0) / clients.length : 0,
    ...
    return (
      <div className='min-h-screen bg-gray-50 flex items-center justify-center'>
        <div className='text-center'>
          <h2 className='text-xl font-semibold text-gray-900 mb-2'>{t('common.error', 'Error Loading Clients')}</h2>
                ...
                <div>
                  {currentPageClients.length === 0 ? (
                    <div className="text-center py-12">
                      <p className="text-gray-500">{t('empty.noClients', 'No clients found matching your criteria.')}</p>
                    </div>
                  ) : (
