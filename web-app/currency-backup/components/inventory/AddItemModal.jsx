...
const AddItemModal = ({ isOpen, onClose, onSave }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) newErrors.name = safeT('modals.addItem.errors.nameRequired', {}, 'Product name is required');
    if (!formData.sku.trim()) newErrors.sku = safeT('modals.addItem.errors.skuRequired', {}, 'SKU is required');
    if (!formData.category) newErrors.category = safeT('modals.addItem.errors.categoryRequired', {}, 'Category is required');
    if (!formData.location) newErrors.location = safeT('modals.addItem.errors.locationRequired', {}, 'Location is required');
    if (!formData.stock || formData.stock < 0) newErrors.stock = safeT('modals.addItem.errors.stockRequired', {}, 'Valid stock quantity is required');
    if (!formData.minStock || formData.minStock < 0) newErrors.minStock = safeT('modals.addItem.errors.minStockRequired', {}, 'Valid minimum stock is required');
    if (!formData.price || formData.price <= 0) newErrors.price = safeT('modals.addItem.errors.priceRequired', {}, 'Valid price is required');
    if (!formData.supplier.trim()) newErrors.supplier = safeT('modals.addItem.errors.supplierRequired', {}, 'Supplier is required');
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          ...
                          >
                            <option value="">{safeT('modals.addItem.placeholders.category', {}, 'Select a category')}</option>
                        ...
                        <div>
                          ...
                          >
                            <option value="">{safeT('modals.addItem.placeholders.location', {}, 'Select a location')}</option>
                        ...
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                          <div className="space-y-1 text-center">
                            ...
                            <div className="flex text-body text-gray-600">
                              ...
                              >
                                <span>{safeT('modals.addItem.imageUpload.uploadFile', {}, 'Upload a file')}</span>
                                <input
                                  id="image"
                                  name="image"
                                  type="file"
